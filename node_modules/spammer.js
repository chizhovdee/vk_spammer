...
var login_timeout = 1000; // одна секунда
var get_group_members_timout = 1000;
var get_users_timeout = 1000;
var add_to_friends_timeout = 5000;
...
function parseGroupsForAddingMembersToFriends(callback){
  ...
  if(!global.current_group) global.current_group = selectGroupForPase();

  if(!global.current_group) return;
          ...
          if(online_users.length > 0){
            ...
            addToFriends(online_user_ids, function(){
              if(response.count > response.offset){
                ...
                callback(); // Здесь другую группу надо выбрать
              }
            });
...
function selectGroupForPase() {
  var group = global.groups[queue_group];
...
//               global.current_user = {}; // сброс пользователя
...
//             if(global.current_user.captcha){
//               global.current_user.captcha += 1;
//             } else {
//               global.current_user.captcha = 1;
//             }
...
//             // var delay = global.current_user.captcha < 10 ? 5000 : global.current_user.captcha * 1000;

//             // if(global.current_user.captcha < 10){
//             //   setTimeout(function(){
//             //     if(users.length > count + 1){
//             //       self.addToFriends(users, count);
//             //     } else {
...
//     function(error, response){
//       if(error){
//         application.secondaryLog('Произошла ошибка получения данных профиля пользователя <strong>' + global.current_user.user_id + '</strong>');
...
//         _.extend(global.current_user, response[0]);

//         application.secondaryLog('Данные получены успешно');
//         //application.secondaryLog(util.inspect(global.current_user));
//         application.secondaryLog('Количество друзей - <strong>' + global.current_user.counters.friends + '</strong>');
...
// _Class.prototype.getMembersOfGroup = function(group_id, count, offset){
//   var self = this;
