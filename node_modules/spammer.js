...
var get_group_members_timout = 500;
var get_users_timeout = 500;
var add_to_friends_timeout = 500; // 60 секунд
...
var add_friend_message = "Привет. Хотел пригласить ознакомиться с творчеством данного коллектива http://vk.com/therapsida. Прошу прощения за спам."
...
function addToFriends(users, callback){
  ...
  var addToFriend = function(user_id, options){
  ...
  var showCaptcha = function(captcha, user_id){
    application.showCaptchaModal(captcha.captcha_img, function(captcha_key){
      addToFriend(user_id, {captcha_key: captcha_key, captcha_sid: captcha.captcha_sid});
    });
  }
...
function vkLogin(callback){
  ...
  if(!auth_data_item) {
    ...
    application.secondaryLog('Завершаем работу программы.');

    return;
  }

  var login = auth_data_item[2];
  var pass  = auth_data_item[3];
